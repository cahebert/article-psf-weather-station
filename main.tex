...
% \documentclass[twocolumn,astrosym]{openjournal}
...
\author[0000-0002-7397-2690]{Claire-Alice H\'ebert}
...
\begin{abstract}
%[250 word limit for the abstract for AAS journal, but not for an open journal we are considering.] 
...
To study the resulting correlations in the size and shape of the PSF, we 
%use the \galsim package to 
generate simulated images of the PSF across a $\approx$ 10 square-degree field of view -- the size of the camera focal plane for the Vera C.~Rubin Observatory in Chile -- using weather data and historical seeing for a geographic location near the Observatory. 
...
%We map this anisotropy from the ground to the sky using model for a possible Rubin observing pattern and find [fill in conclusion once study is done]. 
% We model and interpolate the PSF across the full field of view using several algorithms implemented in the {\sc Piff} package and quantify the residual 2PCFs. 
%We find [summarize conclusion with study is done]. 
...
%and includes installation instructions, documentation, and a tutorial notebook. 
\end{abstract}
...
% historical MASS-DIMM distributions 
...
We describe how the atmosphere and PSFs are modeled in \secref{atmos}, outline the \psfws package in \secref{psfws}, describe three sets of inputs to simulations in \secref{imsiminputs}, define PSF parameters and two-point statistics in \secref{definitions2pcf}, and compare these PSF metrics for the three types of simulation inputs in \secref{imsimresults}.
We end with a discussion of implications for cosmic shear analyses and future work. 

%The \psfws code repository for producing the correlated weather parameters for input to simulations is public \url{https://github.com/LSSTDESC/psf-weather-station} and includes installation instructions, documentation, and a tutorial notebook. 
...
% \footnote{Not to be confused with the wavefront outer scale -- the spatial scale at which correlations in wavefront phase vanish \citep{tokovinin_wavefront_1998, borgnino_estimation_1990}.} 
...
$r_{0,i} =  w_i^{-3/5} r_0$.
%\footnote{In the \galsim implementation, the weight parameter $w_i$ is denoted as $r_{0}\_$weights in the call to \texttt{galsim.Atmosphere}.}
...
%we adopt the meteorologic convention for defining wind direction --  namely, the direction \textit{from} which the wind is blowing.
...
The heavy purple curve in each of the top two plots in \figref{inputs} corresponds to an example time that is representative of the dominant weather conditions. 
...
%\section{PSF Simulations} \label{sec:valid}
...
% \footnote{Although the altitudes of the FA screens are not exactly the same in \bench and \psfwssims, this has little impact because the wind directions are so strongly correlated across phase screens.}   
...
% the square root of the variance $\sigma^2$, where
\begin{equation}
    \sigma = {\rm det}(Q)^{1/4}. 
\end{equation}
...
For calculations of two-point correlation functions for PSF shape parameters, it is most useful to define the complex ellipticity with respect to the separation vector between each pair of PSFs:%\footnote{This coordinate system  has the feature that gravitational lensing due to a spherically symmetric source generates purely tangential shear distortions.} 
\begin{equation}
e = e_t + i e_\times,
\label{eqn:e_deftx}
\end{equation}
...
\includegraphics[width=0.47\textwidth]{f3_output_example.png}
    \caption{
    Spatial distributions of PSF size relative to mean size (left) and PSF ellipticity (right) across a simulated 3.5-\unit{deg^2} exposure generated with the \psfws input parameters for the representative example depicted by the dark curves and dots in \figref{inputs}.   
    The orientation of each line in the right plot corresponds to the orientation of the major axis of the PSF shape, while the length and color contrast are proportional to the magnitude of the ellipticity $e$ defined in \eqnref{e_def12}. 
    \label{fig:output}
    }
\end{figure}
...
\includegraphics[width=0.99\textwidth]{f4_evar_speed.png}
    \caption{Variance of PSF ellipticity $e$ for each simulated exposure versus the ground layer (GL) wind speed in that simulation, for three different sets of inputs to the simulations, described in \secref{imsiminputs}.  
...
In \figref{paramvar}, we plot the variance of PSF ellipticity $e$ (defined in \eqnref{e_def12}) across each simulated exposure versus the input ground-layer wind speed for that exposure.
...
% The fact that the values of $\xi_-(\theta)$ are $\mathcal{O}(10^{-6})$\xspace or less (approximately two orders of magnitude lower than the values of $\xi_+(\theta)$)  indicates that $\langle e_{t,a} e_{t,b}\rangle \approx \langle e_{\times,a} e_{\times,b} \rangle$.  
% However, the median values of $\xi_-(\theta)$ are slightly positive for all three types of inputs to the simulations (\psfwssims, \bench, and \match), indicating an anisotropy that is persistent in the ensemble of simulated exposures. 
...
%The following section started off as section 4.4 in the thesis.
...
These correspond to the region above the white dashed line in each of the lower two panels in \figref{anisoexample}. 
We find $\alpha_{\rm max}$ for each of the 531 simulated exposures for each of the three types of simulations inputs (\psfwssims, \bench, \match) described in \secref{imsiminputs}. 
...
In the scatter plots in \figref{bigscatter}, we plot $\alpha_{\rm max}$ for the shape 2PCF $\xi_+(\alpha)$ versus $\phi(\text{GL})$ (left) and versus $\phi(\text{FA})$ (right), for \largesep (top) and $\sim\,$1\amin (bottom).  
...
%This range of angular scales is approximately the same as the ranges of separation $\theta$ we use to calculate $\xi_+(\alpha)$ and  $C(\alpha)$ in \secref{covariances}, and the associated values of $\alpha_{\rm max}$.
...
%Next, we describe a physical source of correlation of $\alpha_{\rm max}$ at either separation, with wind direction at different altitudes.
...
%$\mathcal O({\rm few} \amin)$ to  $\mathcal O(100 \amin)$ 
from a few arcmin to over 100\amin 
(see \figref{1d2pcf}). 
\item 
There exist dominant wind directions at Rubin Observatory (red histograms in top panels in \figref{bigscatter}), which in turn lead to dominant orientations of anisotropies in the 2PCF $\xi_+$ (red histograms in right panels in \figref{bigscatter}). 
At scales of $\sim100\amin$, 
%$\mathcal O(100 \amin)$, 
these anisotropies are due to strong correlations with ground-layer wind direction (upper left  scatter plot in \figref{bigscatter}); at scales of only a few arcmin, 
%$\mathcal O({\rm few} \amin)$, 
...
%As stated above, unbiased measurements of cosmic shear require accurate corrections of galaxy shapes for the PSF.
%In particular, 
...
% wash out when exposures for a single field on the sky are combined 
...
%%% Some sections were copied/modified from papers submitted by Tianqing Zhang et al. to MNRAS in May 2022 (https://arxiv.org/abs/2205.07892) and by Judit Prat to Open Journal of Astrophysics in Dec 2022 (https://arxiv.org/abs/2212.09345v2).
%%% If appropriate, thank anonymous referee(s) for their helpful feedback on this paper. 
...
%%% DE-SC0009841 is the appropriate number for the Burchat DOE grant. 
...
%%% The following standard acknowledgements were copied from https://github.com/LSSTDESC/desc-tex/blob/master/ack/standard.tex
...
%% Stanford-specific acknowledgement. 
